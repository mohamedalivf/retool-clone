...
export const useEditStore = create<EditStoreType>()(
	devtools(
		persist(
				...
				immer((set, get) => ({
					...
					addComponent: (type: ComponentType, position?: Position) => {
						...
						set((draft) => {
							...
							draft.selection.selectedComponentId = newComponent.id;
							draft.selection.selectionHistory.push(newComponent.id);

							// Auto-open right sidebar when adding component
							draft.sidebars.rightSidebar.isOpen = true;

							// Close left sidebar after adding
							draft.sidebars.leftSidebar.isOpen = false;
						});
					...
					updateDrag: (position: Position) => {
						...
						if (!state.drag.isDragging || !state.drag.draggedComponentId)
							return;
